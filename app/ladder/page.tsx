...
function describe(search: Search): {
  ...
  return {
    ...
    /*
     * Paging is indexable again; facets, sorts and searches are not.
     *
     * The page rule used to be a consent decision — twelve crawlable pages
     * naming non-consenting founders instead of one — and it went away with the
     * agreement that made the sheets themselves indexable. It is also the wrong
     * shape for SEO now: page two of a ladder is the only path a crawler has to
     * the founders ranked 101 and below, and telling it to forget that page
     * while asking it to follow the links on it is a half-instruction.
     *
     * The other three stay out, for reasons that were never about consent. A
     ...
     * search is an unbounded space of pages generated by whoever types into it.
     * Facets are the interesting case and deliberately left for later: "French
     * founders, ranked" is a real query and a real landing page, but a facet
     * grid indexed carelessly is the classic way to bury a small site under its
     * own near-duplicates.
     */
    filtered: Boolean(plain) || q !== '' || search.sort === 'ilvl',
  }
}
