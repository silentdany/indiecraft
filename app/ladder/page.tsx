...
/** How many facets a view narrows by. One is a landing page; more is a long tail. */
function facetCount(search: Search): number {
  return [search.class, search.realm, search.faction, search.ach].filter(
    (v) => (v ?? '').trim() !== '',
  ).length
}
...
function describe(search: Search): {
  ...
  /*
   * A narrowed view leads with its subject, not with "The ladder".
   *
   * These are landing pages now — one facet each is indexed and submitted — and
   * the first words of a title are the ones a search result is judged on.
   * "The ladder — France — founders" spent them on the site's furniture and
   * then read as a list of nouns; "Indie founders on the France realm" is the
   * query somebody typed.
   *
   * `sentence` is reused rather than `named` because it is the one construction
   * that survives every country in the corpus — the realm names refuse a bare
   * preposition ("on France", "on United States") and take the article fine.
   */
  const subjectTitle = badge
    ? plain && narrowed
      ? `${badge.label} — indie ${sentence}`
      : `Founders who earned ${badge.label}`
    : `Indie ${sentence}`

  return {
    title: (plain ? subjectTitle : 'The ladder') + suffix,
    ...
    /*
     * One facet is a landing page. Two is a near-duplicate.
     *
     * "French founders, ranked" and "founders who earned Ten Thousand" are real
     * queries with real answers, and the site has 101 realms, 11 classes, 3
     * factions and 35 badges — around fifty of which hold enough people to be
     * worth reading. Indexed one at a time that is fifty pages. Indexed in
     * combination it is the product of all four, which is the classic way to
     * bury a small site under its own near-duplicates, so a narrowed view with
     * more than one facet on it stays out.
     *
     * Sorts and searches stay out for reasons that were never about consent. A
     * re-sorted ladder is the SAME founders in a different order — duplicate
     * content with a canonical of its own, asking a crawler to keep both. A
     * search is an unbounded space generated by whoever types into it.
     *
     * Population is NOT checked here. A realm with three founders is a thin
     * page, and the answer to a thin page is the same as it was for a thin
     * sheet: it stays indexable and goes unsubmitted. The sitemap holds that
     * threshold, where the counts are already in hand.
     */
    filtered: facetCount(search) > 1 || q !== '' || search.sort === 'ilvl',
  }
}
